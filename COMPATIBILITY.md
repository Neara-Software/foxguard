...
- **String-literal and string-literal-regex sources** — a `pattern-sources` `pattern: "..."` (or a regex-constrained literal) compiles to `LiteralString { regex }`, seeding matching string literals as tainted (e.g. `http://`-prefixed literals for SSRF `request-with-http` rules). A fully-literal non-matching string never seeds.
