...
mod cross_file {
    ...
    /// C# cross-file taint: a `Request.QueryString` source in `Handler.cs`
    /// flows into `QueryHelper.RunQuery(name)`, a helper defined in a sibling
    /// file whose parameter reaches a `SqlCommand` SQL sink. The two-pass
    /// scanner must connect the source to the cross-file sink and emit exactly
    /// one `csharp/taint-sql-injection` finding.
    #[test]
    fn test_csharp_cross_file_taint_directory_scan() {
        let output = foxguard_cmd_isolated()
            .args(["tests/fixtures/realistic/csharp_chain", "-f", "json"])
            .output()
            .expect("failed to execute foxguard");
        ...
        let sql_taint: Vec<&serde_json::Value> = findings
            .iter()
            .filter(|f| f["rule_id"].as_str() == Some("csharp/taint-sql-injection"))
            .collect();

        assert_eq!(
            sql_taint.len(),
            1,
            "csharp/taint-sql-injection should fire exactly once cross-file on csharp_chain; got: {findings:?}"
        );

        // The single finding is reported in the *caller* file (Handler.cs),
        // at the cross-file helper call, and labelled as a cross-file flow.
        let finding = sql_taint[0];
        assert!(
            finding["file"]
                .as_str()
                .is_some_and(|f| f.ends_with("Handler.cs")),
            "cross-file finding should be reported in Handler.cs; got: {finding:?}"
        );
        assert!(
            finding["sink_description"]
                .as_str()
    ...
    /// Negative control for the C# cross-file pass: scanning only the source
    /// file (`Handler.cs`) — without the sibling helper that holds the sink —
    /// must not produce a `csharp/taint-sql-injection` finding. Single-file
    /// scans never run pass 1, so the helper body is unseen and `RunQuery` is
    /// not itself a sink.
    #[test]
    fn test_csharp_cross_file_taint_single_file_finds_nothing() {
        let output = foxguard_cmd_isolated()
            .args([
                "tests/fixtures/realistic/csharp_chain/Handler.cs",
                "-f",
                "json",
            ])
            .output()
        ...
        assert_eq!(
            n, 0,
            "csharp/taint-sql-injection must not fire when Handler.cs is scanned alone; got: {findings:?}"
        );
    }

    /// Kotlin cross-file taint: a `call.receiveText()` source in
    /// `Controller.kt` flows into `CommandHelper.run(cmd)`, a helper defined in
    /// a sibling file whose parameter reaches a `Runtime.exec` command sink.
    /// The two-pass scanner must connect the source to the cross-file sink and
    /// emit exactly one `kt/taint-command-injection` finding, reported in the
    /// caller file and labelled as a cross-file flow.
    #[test]
    fn test_kotlin_cross_file_taint_directory_scan() {
        let output = foxguard_cmd_isolated()
            .args(["tests/fixtures/realistic/kotlin_chain", "-f", "json"])
            .output()
            .expect("failed to execute foxguard");
        ...
        let cmd_taint: Vec<&serde_json::Value> = findings
            .iter()
            .filter(|f| f["rule_id"].as_str() == Some("kt/taint-command-injection"))
            .collect();

        assert_eq!(
            cmd_taint.len(),
            1,
            "kt/taint-command-injection should fire exactly once cross-file on kotlin_chain; got: {findings:?}"
        );

        // The single finding is reported in the *caller* file (Controller.kt),
        // at the cross-file helper call, and labelled as a cross-file flow.
        let finding = cmd_taint[0];
        assert!(
            finding["file"]
                .as_str()
                .is_some_and(|f| f.ends_with("Controller.kt")),
            "cross-file finding should be reported in Controller.kt; got: {finding:?}"
        );
        assert!(
            finding["sink_description"]
                .as_str()
    ...
    /// Negative control for the Kotlin cross-file pass: scanning only the
    /// source file (`Controller.kt`) — without the sibling helper that holds
    /// the sink — must not produce a `kt/taint-command-injection` finding.
    /// Single-file scans never run pass 1, so the helper body is unseen and
    /// `run` is not itself a sink.
    #[test]
    fn test_kotlin_cross_file_taint_single_file_finds_nothing() {
        let output = foxguard_cmd_isolated()
            .args([
                "tests/fixtures/realistic/kotlin_chain/Controller.kt",
                "-f",
                "json",
            ])
            .output()
        ...
        assert_eq!(
            n, 0,
            "kt/taint-command-injection must not fire when Controller.kt is scanned alone; got: {findings:?}"
        );
    }
